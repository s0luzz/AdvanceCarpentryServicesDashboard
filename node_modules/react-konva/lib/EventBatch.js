...
exports.addPendingUnmount = addPendingUnmount;
exports.prepareUnmounts = prepareUnmounts;
exports.getEventBatch = getEventBatch;
exports.scheduleMicrotask = scheduleMicrotask;
exports.batchEvents = batchEvents;
const react_dom_1 = require("react-dom");
const pending = new Set();
const pendingUnmounts = new Set();
const enqueue = typeof queueMicrotask === 'function'
    ? queueMicrotask
    : (callback) => Promise.resolve().then(callback);
let depth = 0;
const batches = new WeakMap();
function addPendingUnmount(callback) {
...
function prepareUnmounts() {
    for (const callback of pendingUnmounts)
        callback();
}
function getEventBatch(batch) {
    if (!batch)
        return batchEvents;
...
function batchEvents(callback) {
    ...
    try {
        ...
        (0, react_dom_1.flushSync)(callback);
    }
    finally {
        try {
